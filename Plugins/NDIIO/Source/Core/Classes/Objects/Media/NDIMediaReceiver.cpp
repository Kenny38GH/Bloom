...
int32 UNDIMediaReceiver::GeneratePCMData(UNDIMediaSoundWave* AudioWave, uint8* PCMData, const int32 SamplesNeeded)
{
	...
	if ((p_framesync_instance != nullptr) && (ConnectionInformation.bMuteAudio == false))
	{
		...
		if (no_samples > 0)
		{
			...
			NDIlib_framesync_capture_audio(p_framesync_instance, &audio_frame, sample_rate, 0,
										   FMath::Min(no_samples, SamplesNeeded));

			// Down-mix to mono if needed
			if (audio_frame.no_channels > 1)
			{
				// Add all channels to channel 0 (normalization done later)
				for (int32 channel_index = 1; channel_index < audio_frame.no_channels; ++channel_index)
				{
					const float* channel_data = reinterpret_cast<const float*>(reinterpret_cast<const uint8 *>(audio_frame.p_data) + channel_index * audio_frame.channel_stride_in_bytes);
					for (int32 sample_index = 0; sample_index < audio_frame.no_samples; ++sample_index)
					{
						audio_frame.p_data[sample_index] += channel_data[sample_index];
					}
				}
			}
			...
			for (int32 sample_index = 0; sample_index < audio_frame.no_samples; ++sample_index)
			{
				...
				int32 sample_int32 = FMath::RoundToInt(audio_frame.p_data[sample_index] / audio_frame.no_channels * 32767.0f);
				...
				PCMData[sample_index * 2 + 0] = sample & 0xff;
				PCMData[sample_index * 2 + 1] = (sample >> 8) & 0xff;
			}

			bytes_generated = audio_frame.no_samples * sizeof(int16);
		...
		{
			const int32 available_samples = 128;

			FMemory::Memset(PCMData, 0, available_samples * sizeof(int16));

			bytes_generated = available_samples * sizeof(int16);
		}
	}
