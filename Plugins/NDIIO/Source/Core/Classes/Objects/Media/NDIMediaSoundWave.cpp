...
bool UNDIMediaSoundWave::InitAudioResource(FName Format)
{
	return true;
}

void UNDIMediaSoundWave::Serialize(FArchive& Ar)
{
	return USoundWave::Serialize(Ar);
}

bool UNDIMediaSoundWave::HasCompressedData(FName Format, ITargetPlatform* TargetPlatform) const
{
	return false;
}

int32 UNDIMediaSoundWave::GetResourceSizeForFormat(FName Format)
{
	return 0;
}

FByteBulkData* UNDIMediaSoundWave::GetCompressedData(FName Format,
