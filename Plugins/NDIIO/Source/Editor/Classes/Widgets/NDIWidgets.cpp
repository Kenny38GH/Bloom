...
void FNDIConnectionInformationCustomization::CustomizeHeader(TSharedRef<IPropertyHandle> InPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	...
	[
		...
		.OnSourceClicked_Lambda([this](FNDIConnectionInformation Source)
		{
			TArray<void*> RawData;
			...
			{
				// Use the source, machine, and stream names
				// Keep the URL clear as it is not persistent
				// Leave the other settings untouched
				ConnectionInformation->SourceName = Source.SourceName;
				ConnectionInformation->MachineName = Source.MachineName;
				ConnectionInformation->StreamName = Source.StreamName;
